"""
generer_dashboard_korridorer.py -- KORRIDOR-VERSJON (Louvain-definerte korridorer)
"""
...
def load_and_process_korridor_data(filepath):
    df = pd.read_csv(filepath, sep=";", decimal=",", encoding="utf-8-sig")
    df.columns = df.columns.str.strip().str.replace('\ufeff', '')
    df["korridor_fra"] = df["korridor_fra"].astype(str).str.strip()
    df["korridor_til"] = df["korridor_til"].astype(str).str.strip()
...
def prepare_korridor_data(df):
    sentra_fra = sorted(df["korridor_fra"].unique().tolist())
    sentra_til = sorted(df["korridor_til"].unique().tolist())
    tider = sorted(df["time_of_day"].unique().tolist())
    kvartaler = df.sort_values("kvartal_sort")["kvartal"].unique().tolist()

    cols = ["korridor_fra", "korridor_til", "kvartal", "reiser", "co2_tonn", "time_of_day", "weekday_indicator"]
    records = df[cols].to_dict("records")

    return {
        "records": records,
        "sentra_fra": sentra_fra,
        "sentra_til": sentra_til,
        "tider": tider,
        "kvartaler": kvartaler
    }
...
def generate_html(ko_data, reiser_data, ko_aggregated, korridor_data, first_ko_date, first_forsinkelser_date, hjelp_html=""):
    ...
    sentrum_fra_options = '<option value="Alle" selected>Alle</option>\n' + \
                          "\n".join(f'<option value="{s}">{s}</option>' for s in korridor_data["sentra_fra"])
    sentrum_til_options = '<option value="Alle" selected>Alle</option>\n' + \
                          "\n".join(f'<option value="{s}">{s}</option>' for s in korridor_data["sentra_til"])

    tid_radios = '<label><input type="radio" name="tid-korridor" value="Alle" checked onchange="updateKorridorChart()"> Alle</label>\n'
    for tid in sorted(korridor_data["tider"]):
        tid_radios += f'<label><input type="radio" name="tid-korridor" value="{tid}" onchange="updateKorridorChart()"> {tid}</label>\n'
...
<head>
    ...
    <title>Mobilitetsdashbord - Nord-Jæren (Korridorer)</title>
...
<body>
    <div class="header"><h1>Mobilitetsdashbord for Nord-Jæren – Korridorer</h1></div>
    <div class="nav">
        ...
        <button onclick="showPage('korridorer')">Reisestrømmer korridorer</button>
    ...
    <div class="container">
        ...
        <div class="sidebar" id="sidebar-reisestatistikk" style="display: none;">
            ...
            <select id="strekning-reiser" onchange="updateReiserChart()">
                {"".join(f'<option value="{s}">{s}</option>' for s in strekninger_reiser)}
            </select>
            <hr>
        ...
        <div class="sidebar" id="sidebar-korridorer" style="display: none;">
            <h3>Velg filtre</h3>
            <label for="sentrum-fra">Korridor fra</label>
            <select id="sentrum-fra" multiple onchange="updateKorridorChart()">{sentrum_fra_options}</select>
            <div class="filter-hint">Ctrl+klikk for flervalg</div>
            <label for="sentrum-til">Korridor til</label>
            <select id="sentrum-til" multiple onchange="updateKorridorChart()">{sentrum_til_options}</select>
            <div class="filter-hint">Ctrl+klikk for flervalg</div>
            <hr>
            <label>Vis:</label>
            <div class="radio-group">
                <label><input type="radio" name="visning-korridor" value="reiser" checked onchange="updateKorridorChart()"> Reiser</label>
                <label><input type="radio" name="visning-korridor" value="co2_sum" onchange="updateKorridorChart()"> CO2-utslipp (sum)</label>
                <label><input type="radio" name="visning-korridor" value="co2_per_reise" onchange="updateKorridorChart()"> CO2 per reise</label>
            </div>
            <hr>
            ...
            <div class="radio-group">
                <label><input type="radio" name="ukedag-korridor" value="Alle" checked onchange="updateKorridorChart()"> Alle</label>
                <label><input type="radio" name="ukedag-korridor" value="Weekday" onchange="updateKorridorChart()"> Ukedag</label>
                <label><input type="radio" name="ukedag-korridor" value="Weekend" onchange="updateKorridorChart()"> Helg</label>
            </div>
        </div>
        <div class="main">
            <div class="page active" id="page-hjem">
                <h2>Velkommen til Mobilitetsdashbordet – Korridorer</h2>
                <p style="margin: 20px 0;">Dette dashbordet viser reisestrømmer mellom Louvain-definerte korridorer på Nord-Jæren.</p>
                <div class="home-grid">
                    ...
                    <div class="home-card" onclick="navigateTo('korridorer')"><h3>🔀 Reisestrømmer korridorer</h3><p>Reisestrømmer mellom Louvain-definerte korridorer.</p></div>
            ...
            <div class="page" id="page-korridorer">
                <div class="chart">
                    <div id="korridor-chart" style="height: 500px;"></div>
                    <div class="chart-buttons">
                        <button id="sankey-korridor-btn" class="sankey-btn" onclick="openKorridorSankeyModal()" style="display: none;">📊 Vis reisestrømmer</button>
                        <button class="sankey-btn" onclick="exportKorridorCSV()">📥 Eksporter CSV</button>
                    </div>
                </div>
            </div>
            <div id="sankey-korridor-modal" class="modal">
                <div class="modal-content">
                    <div class="modal-header"><h2>Reisestrømmer korridorer</h2><span class="modal-close" onclick="closeKorridorSankeyModal()">&times;</span></div>
                    <div class="sankey-controls">
                        <span><strong>Vis retning:</strong></span>
                        <label><input type="radio" name="sankey-korridor-retning" value="fra" checked onchange="updateKorridorSankeyChart()"> Fra valgte korridorer</label>
                        <label><input type="radio" name="sankey-korridor-retning" value="til" onchange="updateKorridorSankeyChart()"> Til valgte korridorer</label>
                    </div>
                    <div id="sankey-korridor-chart" style="height: 600px;"></div>
                </div>
            </div>
            <div class="page" id="page-kart">
                ...
                <a href="https://qgiscloud.com/jaleas/Nord-J_ren_bussvegen_cloud/" target="_blank" class="kart-button">🗺️ Åpne interaktivt kart</a>
            </div>
    ...
    <script>
        ...
        const korridorData = {json.dumps(korridor_data, ensure_ascii=False)};
        ...
        function showPage(page) {{
            ...
            else if (page === 'korridorer') {{ document.getElementById('sidebar-korridorer').style.display = 'block'; updateKorridorChart(); }}
        }}
        ...
        function updateKorridorChart() {{
            const sentrumFraSelect = document.getElementById('sentrum-fra');
            const sentrumTilSelect = document.getElementById('sentrum-til');
            const tidKorridor = document.querySelector('input[name="tid-korridor"]:checked').value;
            const ukedagKorridor = document.querySelector('input[name="ukedag-korridor"]:checked').value;
            const visningKorridor = document.querySelector("input[name='visning-korridor']:checked").value;
            let fraValg = Array.from(sentrumFraSelect.selectedOptions).map(o => o.value);
            let tilValg = Array.from(sentrumTilSelect.selectedOptions).map(o => o.value);
            const fraAlleValgt = fraValg.includes('Alle') || fraValg.length === 0;
            const tilAlleValgt = tilValg.includes('Alle') || tilValg.length === 0;
            let sentraFra = fraAlleValgt ? korridorData.sentra_fra : fraValg;
            let sentraTil = tilAlleValgt ? korridorData.sentra_til : tilValg;
            let splitPå = null, splitSentra = [];
            if (!fraAlleValgt && fraValg.length > 1) {{ splitPå = 'fra'; splitSentra = fraValg; }}
            else if (!tilAlleValgt && tilValg.length > 1) {{ splitPå = 'til'; splitSentra = tilValg; }}

            let filtered = korridorData.records.filter(r => {{
                const fraMatch = sentraFra.includes(r.korridor_fra);
                const tilMatch = sentraTil.includes(r.korridor_til);
                const tidMatch = tidKorridor === 'Alle' || r.time_of_day === tidKorridor;
                const ukedagMatch = ukedagKorridor === 'Alle' || r.weekday_indicator === ukedagKorridor;
            ...
            const processGroup = (sentrum, idx, filterFn) => {{
                const groupFiltered = filtered.filter(filterFn);
                const kvartalData = {{}};
                ...
                const allKvartaler = korridorData.kvartaler;
                let yValues;
                if (visningKorridor === 'co2_per_reise') yValues = allKvartaler.map(k => {{ const d = kvartalData[k]; return d && d.reiser > 0 ? Math.round(d.co2 / d.reiser * 1000 * 100) / 100 : null; }});
                else if (visningKorridor === 'co2_sum') yValues = allKvartaler.map(k => kvartalData[k] ? Math.round(kvartalData[k].co2 * 100) / 100 : null);
                else yValues = allKvartaler.map(k => kvartalData[k] ? Math.round(kvartalData[k].reiser * 100) / 100 : null);
                const trendValues = beregnGlidendeGjennomsnitt(yValues, 5);
                const farge = farger[idx % farger.length];
                traces.push({{ x: allKvartaler, y: yValues, type: 'scatter', mode: 'markers', name: sentrum, marker: {{ color: farge, size: 5, opacity: 0.6 }}, showlegend: false }});
                traces.push({{ x: allKvartaler, y: trendValues, type: 'scatter', mode: 'lines', name: sentrum, line: {{ color: farge, width: 2, shape: 'spline', smoothing: 1.0 }}, connectgaps: true }});
            }};

            if (splitPå === 'fra') splitSentra.forEach((s, idx) => processGroup(s, idx, r => r.korridor_fra === s));
            else if (splitPå === 'til') splitSentra.forEach((s, idx) => processGroup(s, idx, r => r.korridor_til === s));
            else {{
                const kvartalData = {{}};
                filtered.forEach(r => {{
                    if (!kvartalData[r.kvartal]) kvartalData[r.kvartal] = {{ reiser: 0, co2: 0 }};
                ...
                const trendValues = beregnGlidendeGjennomsnitt(yValues, 5);
                traces.push({{ x: allKvartaler, y: yValues, type: 'scatter', mode: 'markers', name: 'Rådata', marker: {{ color: '#636EFA', size: 5, opacity: 0.6 }}, showlegend: false }});
                traces.push({{ x: allKvartaler, y: trendValues, type: 'scatter', mode: 'lines', name: 'Trend', line: {{ color: '#636EFA', width: 2, shape: 'spline', smoothing: 1.0 }}, connectgaps: true }});
            }}

            let titleText, yAxisLabel;
            if (visningKorridor === 'co2_per_reise') {{ titleText = 'CO2-utslipp per reise – korridorer'; yAxisLabel = 'CO2 (kg per reise)'; }}
            else if (visningKorridor === 'co2_sum') {{ titleText = 'CO2-utslipp – korridorer, sum per kvartal'; yAxisLabel = 'CO2 (tonn per kvartal)'; }}
            else {{ titleText = 'Reisestrømmer korridorer – sum reiser per kvartal'; yAxisLabel = 'Antall reiser (1000 per kvartal)'; }}

            const layout = {{ title: titleText, xaxis: {{ title: 'Kvartal', tickangle: -45, type: 'category', categoryorder: 'array', categoryarray: korridorData.kvartaler }}, yaxis: {{ title: yAxisLabel, rangemode: 'tozero' }}, hovermode: 'x unified', legend: {{ x: 0, y: 1.15, orientation: 'h' }} }};
            Plotly.newPlot('korridor-chart', traces, layout, {{responsive: true}});
            const sankeyBtn = document.getElementById('sankey-korridor-btn');
            sankeyBtn.style.display = ((fraAlleValgt && tilAlleValgt) || visningKorridor === 'co2_sum' || visningKorridor === 'co2_per_reise') ? 'none' : 'inline-block';
        }}

        function exportKorridorCSV() {{ alert('CSV-eksport implementert i full versjon'); }}
        function openKorridorSankeyModal() {{ document.getElementById('sankey-korridor-modal').style.display = 'block'; updateKorridorSankeyChart(); }}
        function closeKorridorSankeyModal() {{ document.getElementById('sankey-korridor-modal').style.display = 'none'; }}
        window.onclick = function(event) {{ if (event.target === document.getElementById('sankey-korridor-modal')) closeKorridorSankeyModal(); }}

        function updateKorridorSankeyChart() {{
            const sentrumFraSelect = document.getElementById('sentrum-fra');
            const sentrumTilSelect = document.getElementById('sentrum-til');
            const retning = document.querySelector('input[name="sankey-korridor-retning"]:checked').value;
            let sentraFra = Array.from(sentrumFraSelect.selectedOptions).map(o => o.value);
            let sentraTil = Array.from(sentrumTilSelect.selectedOptions).map(o => o.value);
            const sisteKvartaler = korridorData.kvartaler.slice(-4);
            let filtered = korridorData.records.filter(r => sisteKvartaler.includes(r.kvartal));
            const strommer = {{}};
            if (retning === 'fra') {{
                filtered.filter(r => sentraFra.includes(r.korridor_fra)).forEach(r => {{
                    const key = r.korridor_fra + '|' + r.korridor_til;
                    if (!strommer[key]) strommer[key] = {{ fra: r.korridor_fra, til: r.korridor_til, reiser: 0 }};
                    strommer[key].reiser += r.reiser || 0;
                }});
            }} else {{
                filtered.filter(r => sentraTil.includes(r.korridor_til)).forEach(r => {{
                    const key = r.korridor_fra + '|' + r.korridor_til;
                    if (!strommer[key]) strommer[key] = {{ fra: r.korridor_fra, til: r.korridor_til, reiser: 0 }};
                    strommer[key].reiser += r.reiser || 0;
                }});
            ...
            if (topp10.length === 0) {{ Plotly.newPlot('sankey-korridor-chart', [], {{ title: 'Ingen data' }}); return; }}
            ...
            Plotly.newPlot('sankey-korridor-chart', [trace], {{ title: retning === 'fra' ? 'Reiser FRA valgte korridorer' : 'Reiser TIL valgte korridorer' }}, {{responsive: true}});
        }}
    </script>
</body>
</html>
...
def main():
    ...
    print("\nLaster korridor-data...")
    korridor_df = load_and_process_korridor_data("data/korridorer.csv")
    korridor_data = prepare_korridor_data(korridor_df)
    print(f"  - {len(korridor_df)} rader")
    ...
    print("\nGenererer HTML...")
    html = generate_html(ko_data, reiser_data, ko_aggregated, korridor_data, first_ko_date, first_forsinkelser_date, hjelp_html)

    os.makedirs("docs", exist_ok=True)

    with open("docs/index_korridorer.html", "w", encoding="utf-8") as f:
        f.write(html)

    print(f"\nFerdig! Generert: docs/index_korridorer.html")
